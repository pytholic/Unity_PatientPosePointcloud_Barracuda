...
public class HeadPred: MonoBehaviour
{
    ...
    private async void Start()
    {
        ...
        Debug.Log($"{input[0, 0, 0, 0]}, {input[0, 0, 1, 0]}, {input[0, 0 ,2, 0]}");
    ...
    public static Tensor pcd_to_tensor(PointCloud pcd)
    {
        var tensor = new Tensor(new int[] {1, 1, 3, pcd.Count});
        int i = 0;
        foreach(var point in pcd.Points)
        {
            if(i < pcd.Count)
            {
                tensor[0, 0, 0, i] = (float)point[0];
                tensor[0, 0, 1, i] = (float)point[1];
                tensor[0, 0, 2, i] = (float)point[2];
            }
            i++;
        }
        return tensor;
